...
import type { ExpenseCategoryDto } from '../../../model/expense-categories';
...
export default function NewExpenseCategory() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const { enqueueSnackbar } = useSnackbar();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { onSubmit, key, getInputProps, reset } = useForm<ExpenseCategoryDto>({
    mode: 'uncontrolled',
    initialValues: {
      name: '',
      icon: '',
            ...
            <div className="col">
              <form className="d-flex flex-column gap-3 my-2" onSubmit={onSubmit(handleSubmit)}>
                <TextInput
                  key={key('name')}
                  {...getInputProps('name')}
                  label={t('expenseCategories.new.controls.name')}
                  required
                  ...
                  <div className="row mx-0 gap-3">
                    ...
                    <div className="col-12 col-md px-0">
                      <ColorInput
                        ...
                        label={t('expenseCategories.new.controls.iconColor')}
                        required
                        disabled={isSubmitting}
                      />
                    </div>
